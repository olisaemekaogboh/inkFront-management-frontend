export default {
  language: {
    name: "Hausa",
    switcherLabel: "Harshe",
    selectPlaceholder: "Zaɓi harshe",
  },

  theme: {
    light: "Haske",
    dark: "Duhu",
    system: "Tsarin",
  },

  common: {
    appName: "AgencyPlatform",
    loading: "Ana ɗorawa...",
    checkingSession: "Ana duba zaman...",
    noContentFound: "Ba a sami abun ciki ba.",
    learnMore: "Ƙara koyo",
    explore: "Bincika",
    subscribe: "Biyan kuɗi",
    sendMessage: "Aika saƙo",
    login: "Shiga",
    logout: "Fita",
    register: "Yi rajista",
    home: "Gida",
    services: "Ayyuka",
    projects: "Ayyukan hannu",
    contact: "Tuntuɓi",
    admin: "Mai gudanarwa",
    close: "Rufe",
    menu: "Menu",
    theme: "Tsarin gani",
    light: "Haske",
    dark: "Duhu",
    welcome: "Barka da zuwa",
    viewBlueprint: "Duba tsari",
    allRightsReserved: "Duk haƙƙoƙin an kiyaye.",
    retry: "Sake gwadawa",
    viewDetails: "Duba cikakken bayani",
    viewAll: "Duba duka",
    contactUs: "Tuntuɓe mu",
    backToList: "Koma zuwa jerin",
    viewProject: "Duba aikin hannu",
    viewPortfolio: "Duba ayyukan da muka yi",
    viewCaseStudy: "Duba nazarin harka",
    startProject: "Fara aikin hannu",
    exploreServices: "Bincika ayyukanmu",
  },

  nav: {
    home: "Gida",
    about: "Game da",
    services: "Ayyuka",
    portfolio: "Ayyukan da muka yi",
    products: "Kayayyaki",
    blog: "Bulogi",
    clients: "Abokan ciniki",
    contact: "Tuntuɓi",
    admin: "Admin",
    login: "Shiga",
    logout: "Fita",
    account: "Asusu",
    dashboard: "Dashboard",
    menu: "Menu",
    openMenu: "Bude menu",
    closeMenu: "Rufe menu",
  },

  footer: {
    description:
      "InkFront yana gina manyan gidajen yanar gizo, tsarin ajiya, dashboards, bulogi, wasiƙun labarai, hanyoyin shiga, da kayan aikin software na musamman don kasuwancin zamani.",
    explore: "Bincika",
    company: "Kamfani",
    startTitle: "Fara da InkFront",
    startText:
      "Shin kuna shirye don haɓaka kasuwancin ku akan layi? Bari InkFront ta gina tsarin dijital mai tsabta, na zamani don alamar ku.",
    contactButton: "Tuntuɓi InkFront",
    phone: "+234 903 017 5230",
    email: "info@inkfront.com",
    address: "4 Garden Street, Fegge, Onitsha, Anambra, Nigeria",
    viewMap: "Duba akan Taswirorin Google",
    rights: "Duk haƙƙoƙin an kiyaye.",
    terms: "Sharuɗɗa",
    privacy: "Sirri",
    contact: "Tuntuɓi",
    cookies: "Kukis",
    termsConditions: "Sharuɗɗa & Yanayi",
    privacyPolicy: "Manufar Sirri",
    cookiesPolicy: "Manufar Kukis",
  },

  auth: {
    loginTitle: "Shiga",
    registerTitle: "Ƙirƙiri asusu",
    loginTab: "Shiga",
    registerTab: "Yi rajista",
    loginButton: "Shiga",
    loginSubmitting: "Ana shiga...",
    registerButton: "Ƙirƙiri asusu",
    registerSubmitting: "Ana ƙirƙirar asusu...",
    emailOrUsername: "Imel ko sunan mai amfani",
    password: "Kalmar sirri",
    firstName: "Suna na farko",
    lastName: "Suna na ƙarshe",
    username: "Sunan mai amfani",
    email: "Imel",
  },

  cookies: {
    eyebrow: "Kukis",
    title: "Manufar Kukis",
    updatedDate: "Afrilu 28, 2026",
    description:
      "Wannan manufar ta bayyana yadda InkFront ke amfani da kukis, ma'ajiyar gida, ma'ajiyar zaman, da makamantan fasahohi don tallafawa tsaro, tabbatarwa, zaɓaɓɓuka, aiki, da ingantaccen ƙwarewar mai amfani.",
    lastUpdated: "Ƙarshen Sabuntawa",
    lastUpdatedDescription:
      "Muna amfani da kukis a hankali don tallafawa shiga mai aminci, samun damar mai gudanarwa, zaɓaɓɓuka, zaɓin yarda, nazari, da ingantaccen hali na dandalin InkFront.",
    ctaEyebrow: "Zaɓinku",
    ctaTitle: "Kuna sarrafa saitunan kukis ɗinku",
    ctaDescription:
      "Kuna iya share kukis daga burauzarku, ƙin kukis marasa mahimmanci inda akwai, ko tuntuɓar InkFront idan kuna da tambayoyi game da keɓantawa, kukis, ko sarrafa bayanai.",
    privacyButton: "Manufar Keɓantawa",
    contactButton: "Tuntuɓi InkFront",
    sections: {
      whatAreCookies: {
        title: "1. Menene Kukis?",
        body: "Kukis ƙananan fayilolin rubutu ne da ake ajiyewa akan na'urarku lokacin da kuka ziyarci gidan yanar gizo. Suna taimaka wa gidajen yanar gizo su tuna bayanai game da ziyararku, kamar zaman shiga, harshe da aka zaɓa, zaɓin tsarin gani, duban tsaro, da halayen amfani na asali. Fasahohi makamantan su na iya haɗawa da ma'ajiyar gida, ma'ajiyar zaman, pixels, tags, abubuwan gano na'ura, da ma'ajiyar zaɓaɓɓuka na tushen burauza.",
      },
      howInkFrontUses: {
        title: "2. Yadda InkFront Ke Amfani da Kukis",
        body: "InkFront na amfani da kukis da makamantan fasahohi don sanya gidan yanar gizonmu ya kasance mai aiki, amintacce, na musamman, da sauƙin amfani. Muna iya amfani da kukis don tabbatarwa, samun damar dashboard na mai gudanarwa, zaman amintattu, zaɓin harshe, zaɓin yanayin duhu ko haske, matsayin yarda da kuki, kariyar fam ɗin tuntuɓa, inganta aiki, da aikin gidan yanar gizo na asali.",
      },
      essential: {
        title: "3. Kukis Masu Muhimmanci",
        body: "Ana buƙatar kukis masu mahimmanci don mahimman fasali na gidan yanar gizo da dandamali suyi aiki daidai. Waɗannan na iya haɗawa da kukis na shiga, kukis na tsaro, kukis na zaman, kariya ta CSRF, alamun tabbatarwa, ikon samun damar mai gudanarwa, da rikodin yarda da kuki. Saboda waɗannan kukis suna da mahimmanci don gidan yanar gizo yayi aiki lafiya, ba za a iya kashe su ta gidan yanar gizonmu ba tare da shafar fasali na asali ba.",
      },
      authSecurity: {
        title: "4. Kukis na Tabbatarwa da Tsaro",
        body: "Inda InkFront ke amfani da fasali na shiga ko dashboard na mai gudanarwa, ana iya amfani da kukis don tabbatar da ainihin mai amfani, kare zaman, hana samun dama mara izini, da kiyaye masu amfani a amince sun shiga. Wasu kukis na tabbatarwa na iya zama HTTP-kawai, ma'ana an tsara su don rubutun burauza ba za su iya samun damarsu kai tsaye ba. Wannan yana taimakawa inganta tsaron asusu.",
      },
      preference: {
        title: "5. Kukis na Zaɓaɓɓuka",
        body: "Kukis na zaɓaɓɓuka suna taimakawa tuna zaɓukan da kuke yi akan gidan yanar gizo, kamar harshe, yanki, zaɓin shimfiɗa, yanayin duhu, yanayin haske, da zaɓukan banner ɗin kuki. Waɗannan kukis suna inganta ƙwarewar mai amfani ta hanyar hana ku sake zaɓar zaɓuɓɓukan iri ɗaya akai-akai.",
      },
      functional: {
        title: "6. Ma'ajiyar Aiki",
        body: "InkFront na iya amfani da ma'ajiyar burauza don tallafawa fasali na dubawa, zaɓaɓɓukan da aka adana, yanayin fam na ɗan lokaci, halayen dashboard, da kewayawa mai sauƙi. Misali, zaɓin tsarin gani ko zaɓin harshe na iya zama an adana a gida don gidan yanar gizo ya iya ɗorawa tare da saitunan da kuka fi so a ziyarori na gaba.",
      },
      analytics: {
        title: "7. Nazari da Inganta Ayyuka",
        body: "InkFront na iya amfani da kayan aikin nazari a yanzu ko a nan gaba don fahimtar yadda baƙi ke mu'amala da gidan yanar gizo, waɗanne shafuka ake ziyarta, yadda masu amfani ke kewayawa dandamali, da yadda za a iya inganta aiki. Ana amfani da bayanan nazari gabaɗaya don inganta abun ciki, shimfiɗa, gudu, ƙwarewar mai amfani, tasirin talla, da ingancin ayyuka.",
      },
      thirdParty: {
        title: "8. Kukis na Ƙungiyoyi na Uku",
        body: "Wasu fasali na gidan yanar gizo na iya dogara ga ayyuka na ƙungiyoyi na uku kamar shiga ta Google, masu samar da masauki, dandamali na imel, kayan aikin nazari, bidiyon da aka saka, taswirori, hanyoyin biyan kuɗi, kayan aikin tallafawa abokan ciniki, ko dandamali na talla. Waɗannan masu samarwa na ƙungiyoyi na uku na iya saita nasu kukis ko fasahohi makamantan su. Kukis ɗinsu ana sarrafa su ta hanyar nasu manufofin keɓantawa da kukis, ba ta InkFront ba.",
      },
      clientProjects: {
        title: "9. Kukis da Ake Amfani da Su a Ayyukan Abokan Ciniki",
        body: "Idan InkFront ya gina gidan yanar gizo, dashboard, portal, ko dandamali na software don abokin ciniki, wannan aikin na iya amfani da kukis don shiga, zaman abokan ciniki, halayen cart, tsarin biyan kuɗi, samun damar mai gudanarwa, nazari, zaɓaɓɓuka, da tsaro. Abokin ciniki ne ke da alhakin tabbatar da cewa nasu sanarwar kuki, manufofin keɓantawa, da bayanan shari'a sun dace da kasuwancinsu, masu amfani, da ikon shari'a.",
      },
      managing: {
        title: "10. Sarrafa Kukis",
        body: "Kuna iya sarrafa kukis ta hanyar saitunan burauzarku. Yawancin burauzoji suna ba ku damar toshe kukis, share kukis, share bayanan shafi, ko karɓar faɗakarwa lokacin da ake amfani da kukis. Lura cewa kashe kukis masu mahimmanci na iya haifar da shiga, samun damar dashboard na mai gudanarwa, fam, zaɓaɓɓuka, ko wasu fasali na asali su daina aiki yadda ya kamata.",
      },
      consent: {
        title: "11. Yarda da Kuki",
        body: "Inda ake buƙata, InkFront na iya nuna banner ɗin kuki ko sanarwar yarda wanda ke ba masu amfani damar karɓa ko ƙin kukis marasa mahimmanci. Ana iya amfani da kukis masu mahimmanci koda kuwa an ƙi kukis marasa mahimmanci saboda ana buƙatar su don tsaro, tabbatarwa, zaɓaɓɓuka, da aikin gidan yanar gizo na asali.",
      },
      retention: {
        title: "12. Riƙewar Kukis",
        body: "Wasu kukis suna ɗorewa kawai don zaman burauzarku na yanzu kuma ana share su lokacin da kuka rufe burauzarku. Wasu kukis na iya kasancewa na tsawon lokaci don gidan yanar gizo ya iya tuna zaɓukanku. Tsawon lokaci ya dogara da manufar kuki, saitunan burauza, da ko InkFront ne ko mai samarwa na ƙungiya ta uku ya saita kuki.",
      },
      updates: {
        title: "13. Sabuntawa ga Wannan Manufar Kukis",
        body: "InkFront na iya sabunta wannan Manufar Kukis daga lokaci zuwa lokaci yayin da gidan yanar gizonmu, ayyuka, ayyukan tsaro, kayan aikin nazari, ko buƙatun shari'a suke canzawa. Za a kasance da sabon sigar akan wannan shafi, kuma ci gaba da amfani da gidan yanar gizo yana nufin kun yarda da manufar da aka sabunta.",
      },
      contact: {
        title: "14. Tuntuɓi",
        body: "Don tambayoyi game da kukis, keɓantawa, fasahohin bin diddigi, ko sarrafa bayanai, don Allah tuntuɓi InkFront ta hanyar shafin tuntuɓa na hukuma ko duk wata hanyar sadarwa da aka tabbatar da InkFront ta samar.",
      },
    },
  },
  terms: {
    eyebrow: "Shari'a",
    title: "Sharuɗɗa & Yanayi",
    updatedDate: "Afrilu 28, 2026",
    description:
      "Waɗannan Sharuɗɗan suna tsara amfani da ku na ayyukan InkFront, gidajen yanar gizo, shawarwari, samfuran dijital, tsarin software, da tsare-tsaren tallafi. Da fatan za a karanta su a hankali kafin fara aiki.",
    lastUpdatedLabel: "Ƙarshen Sabuntawa",
    lastUpdatedDescription:
      "An tsara waɗannan Sharuɗɗan don kare duka InkFront da abokan cinikinmu ta hanyar kafa ƙa'idodi bayyanannu don iyakar aikin, biyan kuɗi, mallaka, tallafi, amfani mai karɓuwa, da nauyin ayyuka.",
    ctaEyebrow: "Kuna Bukatar Ƙarin Bayani?",
    ctaTitle: "Tambayoyi game da sharuɗɗanmu?",
    ctaDescription:
      "Tuntuɓi InkFront kafin fara aikin ku idan kuna buƙatar ƙarin bayani game da iyaka, biyan kuɗi, mallaka, tallafi, ko abubuwan da za a bayar.",
    contactButton: "Tuntuɓi InkFront",
    homeButton: "Koma Gida",
    sections: {
      acceptance: {
        title: "1. Karɓar Sharuɗɗa",
        body: "Ta hanyar shiga, bincike, nema, siye, biyan kuɗi, ko amfani da kowane gidan yanar gizo na InkFront, shawara, sabis ɗin ƙira, sabis ɗin software, samfurin dijital, tsarin gudanarwa, dashboard, tallafin masauki, tsarin kulawa, ko sabis ɗin kasuwanci mai alaƙa, kun yarda a ɗaure ku bisa doka da waɗannan Sharuɗɗa da Yanayi.",
      },
      about: {
        title: "2. Game da InkFront",
        body: "InkFront yana samar da gidajen yanar gizo na kasuwanci, shafukan saukar samfur, tsarin ajiya, hanyoyin shiga abokan ciniki, dashboards na gudanarwa, kayan aikin CRM, shafukan labarai, wasiƙun labarai, dandamali na e-commerce, tsarin sarrafa makarantu, tallafin alama, sarrafa kansa na ayyuka, da mafita na software na musamman.",
      },
      clientResponsibilities: {
        title: "3. Nauyin Abokin Ciniki",
        body: "Dole ne abokan ciniki su samar da ingantattun bayanai, buƙatun kasuwanci, amincewa a rubuce, abun ciki, kadarorin alama, hotuna, bayanan shiga, cikakkun samfur, farashi, manufofi, martani, da duk wani abu da ake buƙata don kammala aikin.",
      },
      scope: {
        title: "4. Iyakar Aikin, Shawarwari & Abubuwan da za a Bayar",
        body: "Ana isar da duk ayyuka bisa ga shawarar da aka amince, daftari, ƙididdiga, kwangila, saƙon rubutu, ko iyakar aikin da aka amince. Duk wani fasali, shafi, haɗin kai, dashboard, tafiyar aiki, abun ciki, sarrafa kansa, ko buƙatar ƙira da ba a haɗa a fili cikin iyakar da aka amince ba za a ɗauke shi azaman ƙarin aiki.",
      },
      payments: {
        title: "5. Biyan Kuɗi, Kuɗin Farko & Lissafin Kuɗi",
        body: "Dole ne a biya kuɗin aikin, kuɗin farko, masu riƙewa, biyan kuɗi, kuɗin kulawa, biyan kuɗi na mataki, da kuɗin sabuntawa bisa ga jadawalin da aka amince.",
      },
      latePayments: {
        title: "6. Jinkirin Biyan Kuɗi & Dakatarwa",
        body: "Idan biyan kuɗi ya wuce lokaci, InkFront na iya dakatar da haɓakawa, tallafin masauki, kulawa, damar gudanarwa, turawa, sabuntawa, ko miƙawa har sai an biya kuɗin da suka rage.",
      },
      revisions: {
        title: "7. Bitoci & Buƙatun Canji",
        body: "Ana iya haɗa bitoci masu ma'ana kawai inda aka bayyana a cikin yarjejeniyar aikin. Dole ne bitoci su shafi iyakar da aka amince da ita da kuma jagorar asali.",
      },
      approvals: {
        title: "8. Amincewar Abokin Ciniki",
        body: "Lokacin da abokin ciniki ya amince da ƙira, fasali, shafi, sashin abun ciki, tafiyar aiki, ko mataki, InkFront na iya ci gaba bisa wannan amincewar.",
      },
      ip: {
        title: "9. Mallakar Hankali",
        body: "Bayan cikakken biyan kuɗi, abokin ciniki yana karɓar mallakar abubuwan da aka biya na ƙarshe waɗanda aka ƙirƙira musamman don abokin ciniki, ban da kayan aikin da suka gabata na InkFront, lambar da za a iya sake amfani da ita, samfura, ɗakunan karatu, tsarukan gini, tsarin ciki, matakai, ilmi, dabaru, tsarin sarrafa kansa, da hanyoyin fasaha na gaba ɗaya.",
      },
      portfolio: {
        title: "10. Haƙƙoƙin Fayil",
        body: "Sai dai idan abokin ciniki ya nemi sirri a rubuce kafin ƙaddamar da aikin, InkFront na iya nuna ayyukan da aka kammala, hotunan aikin, sunan abokin ciniki, tambarin alama, bayanin aikin, da sakamako na gaba ɗaya a cikin fayil ɗinsa.",
      },
      confidentiality: {
        title: "11. Sirri",
        body: "Bangarorin biyu sun yarda su kula da bayanan kasuwanci, fasaha, kuɗi, abokin ciniki, ayyuka, shiga, da aikin sirri cikin hankali.",
      },
      hosting: {
        title: "12. Masauki, Yankuna & Ayyukan Ɓangare na Uku",
        body: "Ayyuka na iya dogara ga ayyukan ɓangare na uku kamar masu samar da masauki, masu rajistar yanki, hanyoyin biyan kuɗi, masu samar da imel, kayan aikin nazari, APIs, masu samar da SMS, ma'ajiyar girgije, ma'ajin bayanai, plugins, ɗakunan karatu, da dandamali na waje.",
      },
      security: {
        title: "13. Tsaro & Dama",
        body: "InkFront zai yi amfani da matakan fasaha masu ma'ana don gina tsarukan amintattu, amma babu wani gidan yanar gizo, software, muhallin masauki, ko dandamali mai haɗin intanet da za a iya ba da tabbacin zama cikakke daga raunuka, hare-hare, asarar bayanai, ko shiga mara izini.",
      },
      compliance: {
        title: "14. Abun Ciki & Biyan Dokoki",
        body: "Abokin ciniki ne kaɗai ke da alhakin tabbatar da cewa abun cikin gidan yanar gizonsa, samfura, ayyuka, da'awar, farashi, kafofin watsa labarai, sanarwar sirri, ayyukan kasuwanci, da sadarwar abokin ciniki sun bi dokoki da ƙa'idodi masu aiki.",
      },
      use: {
        title: "15. Amfani Mai Karɓuwa",
        body: "Kada abokan ciniki su yi amfani da ayyukan InkFront don zamba, yaudara, kwaikwaya, ciniki ba bisa ƙa'ida ba, keta haƙƙin mallaka, spam, malware, phishing, abun ciki na ƙiyayya, tsangwama, cin zarafin manya, ayyukan kuɗi ba bisa ƙa'ida ba, ko duk wani aiki da ke cutar da masu amfani.",
      },
      availability: {
        title: "16. Samuwar Sabis",
        body: "InkFront yana nufin samar da ayyuka masu dogaro, amma baya ba da garantin samun dama ba tare da katsewa ba, aiki mara kuskure, lokacin aiki na dindindin, ko ci gaba da samuwa.",
      },
      support: {
        title: "17. Tallafi & Kulawa",
        body: "Ana haɗa tallafi, sabuntawa, gyaran kurakurai, madogara, sa ido, sabunta abun ciki, inganta fasali, da kulawar fasaha ne kawai inda aka bayyana a cikin yarjejeniyar aikin ko tsarin kulawa mai aiki.",
      },
      liability: {
        title: "18. Iyakancin Alhaki",
        body: "Zuwa iyakar da doka ta yarda, InkFront ba zai zama abin dogaro ba don asarar kai tsaye, na bazata, na musamman, na hukunci, ko na sakamako, gami da asarar riba, asarar kudaden shiga, asarar bayanai, asarar abokan ciniki, lalacewar suna, katsewar kasuwanci, ko damar da aka rasa.",
      },
      indemnity: {
        title: "19. Rammako",
        body: "Abokin ciniki ya yarda ya rama kuma ya riƙe InkFront ba tare da wani lahani ba daga da'awar, lalacewa, hukunci, asara, alhaki, farashi, da kashe kuɗi da suka taso daga kayan da abokin ciniki ya samar, ayyukan kasuwanci ba bisa ƙa'ida ba, rashin amfani da dandamalin da aka isar, keta waɗannan Sharuɗɗan, keta haƙƙin ɓangare na uku, ko gazawar bin dokoki masu aiki.",
      },
      cancellation: {
        title: "20. Sokewa & Ƙarewa",
        body: "Kowane bangare na iya ƙare aikin bisa ga kwangilar da aka amince ko tsarin rubutu. Idan abokin ciniki ya soke aikin bayan an fara aiki, abokin ciniki ya ci gaba da kasancewa da alhakin aikin da aka riga aka kammala, albarkatun da aka ware, farashin ɓangare na uku da aka jawo, da duk wasu kuɗaɗen da ba za a mayar ba.",
      },
      forceMajeure: {
        title: "21. Ƙarfin Fiye da Iko",
        body: "InkFront ba zai zama abin dogoro ba don jinkiri ko gazawa da abubuwan da suka wuce iko mai ma'ana suka haifar, gami da gazawar wutar lantarki, katsewar intanet, bala'in halitta, aikin gwamnati, yajin aiki, yaƙi, tashin hankalin jama'a, rashin lafiya, katsewar dandamali, harin intanet, gazawar mai samar da biyan kuɗi, ko katsewar sabis na ɓangare na uku.",
      },
      law: {
        title: "22. Dokar da ke Mulki",
        body: "Za a fassara waɗannan Sharuɗɗan bisa ga dokokin da suka dace na Jamhuriyar Tarayyar Najeriya, sai dai idan wata yarjejeniya ta rubutu ta ce akasin haka.",
      },
      updates: {
        title: "23. Sabuntawa ga Sharuɗɗa",
        body: "InkFront na iya sabunta waɗannan Sharuɗɗan daga lokaci zuwa lokaci don nuna canje-canje a ayyuka, dokoki, ayyukan kasuwanci, farashi, fasaha, ko buƙatun tsaro.",
      },
      contact: {
        title: "24. Tuntuɓi",
        body: "Don tambayoyi, korafe-korafe, ƙarin bayani kan aikin, al'amuran lissafin kuɗi, ko sanarwar doka da suka shafi waɗannan Sharuɗɗan, da fatan za a tuntuɓi InkFront ta hanyar shafin tuntuɓa na hukuma ko duk wata hanyar sadarwa da aka tabbatar da InkFront ta samar.",
      },
    },
  },
  privacy: {
    eyebrow: "Sirri",
    title: "Manufar Sirri",
    updatedDate: "Afrilu 28, 2026",
    description:
      "Wannan manufar ta bayyana yadda InkFront ke tattara, amfani, adanawa, kiyayewa, da sarrafa bayanan sirri a duk faɗin gidajen yanar gizonmu, fam ɗin tuntuɓa, kayan aikin CRM, wasiƙun labarai, tsarin software, da ayyukan abokin ciniki.",
    lastUpdatedLabel: "Ƙarshen Sabuntawa",
    lastUpdatedDescription:
      "Sirrinka yana da mahimmanci ga InkFront. Muna tattara bayanai ne kawai inda zai taimaka mana mu amsa maka, isar da ayyuka, kiyaye dandamalinmu, sarrafa ayyuka, da inganta ƙwarewar abokin ciniki.",
    ctaEyebrow: "Buƙatar Sirri",
    ctaTitle: "Kuna buƙatar taimako da bayanan sirrinku?",
    ctaDescription:
      "Tuntuɓi InkFront idan kuna son samun dama, gyara, sabuntawa, ƙuntatawa, ko buƙatar share bayanan sirri da ke da alaƙa da tambayarku, biyan kuɗin wasiƙar labarai, ko aiki.",
    contactButton: "Tuntuɓi InkFront",
    termsButton: "Duba Sharuɗɗa",
    sections: {
      introduction: {
        title: "1. Gabatarwa",
        body: "Wannan Manufar Sirri ta bayyana yadda InkFront ke tattara, amfani, adanawa, kiyayewa, da sarrafa bayanan sirri.",
      },
      information: {
        title: "2. Bayanan da Muke Tattarawa",
        body: "InkFront na iya tattara bayanan da kuke bayarwa kai tsaye, gami da sunanka, imel, waya, kamfani, cikakkun aikin, da bayanan fasaha kamar adireshin IP da nau'in burauza.",
      },
      usage: {
        title: "3. Yadda Muke Amfani da Bayananku",
        body: "Muna amfani da bayananku don amsa tambayoyi, isar da ayyuka, sarrafa ayyuka, aika sabuntawa, inganta gidan yanar gizonmu, da kiyaye tsaro.",
      },
      contactForms: {
        title: "4. Fam ɗin Tuntuɓa da Saƙonnin CRM",
        body: "Ana iya adana abubuwan da aka gabatar ta fam ɗin tuntuɓa a cikin tsarin CRM ɗinmu don bita, aiki, bin diddigi, da sarrafa amsa.",
      },
      email: {
        title: "5. Imel da Sanarwa",
        body: "Muna iya aika imel masu alaƙa da sabis gami da sabuntawar aiki, bibiyar shawarwari, amsoshin tallafi, da sanarwa masu mahimmanci.",
      },
      newsletter: {
        title: "6. Biyan Kuɗi na Wasiƙar Labarai",
        body: "Biyan kuɗi na wasiƙar labarai yana adana imel ɗinku, suna, zaɓin harshe, da matsayin biyan kuɗi. Kuna iya cire rajista kowane lokaci.",
      },
      cookies: {
        title: "7. Kukis da Tabbatarwa",
        body: "Muna amfani da kukis don tabbatarwa, zaman amintattu, zaɓaɓɓuka, tsaro, nazari, da aikin gidan yanar gizo na asali.",
      },
      admin: {
        title: "8. Asusun Gudanarwa da Samun Dashboard",
        body: "Samun dashboard ɗin gudanarwa ya haɗa da sarrafa bayanan shiga, matsayin mai amfani, rijistar ayyuka, da bayanan zaman don kare wuraren da aka ƙuntata.",
      },
      projectData: {
        title: "9. Aiki da Bayanan Abokin Ciniki",
        body: "Abun cikin kasuwanci da abokin ciniki ya samar, tambura, hotuna, da bayanan fasaha ana amfani da su ne kawai kamar yadda ake buƙata don isar da aikin.",
      },
      security: {
        title: "10. Tsaron Bayanai",
        body: "Muna ɗaukar matakan da suka dace don kare bayanan sirri, kodayake babu dandamali mai haɗin intanet da za a iya ba da tabbacin zama cikakke amintacce.",
      },
      thirdParty: {
        title: "11. Ayyukan Ɓangare na Uku",
        body: "Muna amfani da ayyukan ɓangare na uku kamar masu samar da masauki, imel, nazari, da masu samar da biyan kuɗi waɗanda ke sarrafa bayanai bisa ga nasu manufofin.",
      },
      payment: {
        title: "12. Biyan Kuɗi da Bayanan Lissafin Kuɗi",
        body: "Muna sarrafa cikakkun bayanan lissafin kuɗi da rikodin biyan kuɗi. Ba a adana cikakkun bayanan kati da gangan ba sai dai idan mai samar da biyan kuɗi mai aminci ya kula.",
      },
      legalBasis: {
        title: "13. Tushen Shari'a da Amfani Halaltacce",
        body: "Muna sarrafa bayanai inda ya zama dole don ayyuka, kwangiloli, wajibai na shari'a, rigakafin zamba, ko halaltattun muradun kasuwanci.",
      },
      retention: {
        title: "14. Riƙewar Bayanai",
        body: "Ana riƙe bayanai ne kawai muddin ya zama dole don kasuwanci, shari'a, lissafin kuɗi, tsaro, ko dalilai masu alaƙa da aiki.",
      },
      sharing: {
        title: "15. Rarraba Bayanai",
        body: "Ba mu sayar da bayanan sirrinku. Muna rarrabawa ne kawai inda ya zama dole tare da ƙungiya, abokan hulɗa, masu samarwa, ko hukumomi kamar yadda ake buƙata.",
      },
      international: {
        title: "16. Sarrafa na Ƙasashen Duniya",
        body: "Ana iya sarrafa bayanai a wasu ƙasashe ta hanyar masauki ko masu samar da ayyuka. Muna aiki tare da masu samarwa masu daraja kuma muna amfani da ƙa'idodin tsaro masu amfani.",
      },
      rights: {
        title: "17. Haƙƙoƙinku",
        body: "Kuna iya buƙatar samun dama, gyara, sharewa, ko ƙuntata bayananku inda doka ta yarda.",
      },
      children: {
        title: "18. Sirrin Yara",
        body: "Ayyukanmu na kasuwanci ne da manya. Ba mu tattara bayanai daga yara da gangan ba.",
      },
      links: {
        title: "19. Hanyoyin Zuwa Wasu Gidajen Yanar Gizo",
        body: "Ba mu da alhakin ayyukan sirri na gidajen yanar gizo na waje waɗanda aka haɗa daga dandamalinmu.",
      },
      changes: {
        title: "20. Canje-canje ga Wannan Manufar Sirri",
        body: "Muna iya sabunta wannan manufar. Ci gaba da amfani yana nufin yarda da sigar da aka sabunta.",
      },
      contact: {
        title: "21. Tuntuɓi",
        body: "Don tambayoyin sirri ko buƙatun bayanai, tuntuɓi InkFront ta hanyar shafin tuntuɓa na hukuma.",
      },
    },
  },
  admin: {
    shell: {
      title: "Dashboard na mai gudanarwa",
      subtitle: "Sarrafa abun ciki, tsari, da saitunan shafi.",
      welcome: "Barka da dawowa, {name}",
    },
    states: {
      loading: "Ana ɗorawa bayanai...",
      empty: "Ba a sami bayanai ba.",
    },
    actions: {
      create: "Ƙirƙiri sabo",
      edit: "Gyara",
      delete: "Share",
      cancel: "Soke",
      save: "Ajiye",
      saving: "Ana ajiyewa...",
      deleting: "Ana sharewa...",
    },
    pagination: {
      previous: "Wanda ya gabata",
      next: "Na gaba",
      page: "Shafi",
    },
    confirmDelete: {
      title: "Share bayani",
      description: "Ba za a iya sake wannan aikin ba.",
    },
    dashboard: {
      title: "Ƙididdiga",
      description: "Hoton gaggawa na abun cikin shafi a duk sassa.",
      loading: "Ana ɗorawa dashboard...",
    },
    menu: {
      dashboard: "Ƙididdiga",
      services: "Ayyuka",
      portfolioProjects: "Ayyukan hannu na portfolio",
      productBlueprints: "Tsare-tsaren kayayyaki",
      testimonials: "Shaidar",
      clientLogos: "Tambarin abokan ciniki",
      homepageSections: "Sassan shafin gida",
      contactSubmissions: "Abubuwan tuntuɓa da aka gabatar",
      newsletterSubscribers: "Masu biyan kuɗi na wasiƙar labarai",
      translations: "Fassarori",
      siteSettings: "Saitunan shafi",
      navigationItems: "Kewayawa",
      socialLinks: "Hanyoyin sadarwar zamantakewa",
      mediaAssets: "Kayan watsa labarai",
      faqs: "Tambayoyin da ake yawan yi",
      categories: "Rukunoni",
    },
    columns: {
      actions: "Ayyuka",
      name: "Suna",
      slug: "Slug",
      category: "Rukuni",
      featured: "Wanda aka fifita",
      published: "An buga",
      title: "Take",
      type: "Nau'i",
      clientName: "Abokin ciniki",
      organization: "Ƙungiya",
      imageUrl: "URL na hoto",
      sectionKey: "Maɓallin sashe",
      placement: "Matsayi",
      fullName: "Cikakken suna",
      email: "Imel",
      subject: "Maudu'i",
      status: "Matsayi",
      active: "Yana aiki",
      language: "Harshe",
      namespace: "Namespace",
      translationKey: "Maɓallin fassara",
      value: "Ƙima",
      group: "Ƙungiya",
      key: "Maɓalli",
      label: "Lakabi",
      url: "URL",
      location: "Wuri",
      visible: "Ana iya gani",
      platform: "Dandamali",
      assetType: "Nau'in kadara",
      question: "Tambaya",
      pageKey: "Maɓallin shafi",
    },
    fields: {
      name: "Suna",
      slug: "Slug",
      shortDescription: "Takaitaccen bayani",
      fullDescription: "Cikakken bayani",
      category: "Rukuni",
      iconKey: "Maɓallin gumaka",
      featured: "Wanda aka fifita",
      published: "An buga",
      displayOrder: "Tsarin nuni",
      language: "Harshe",
      title: "Take",
      summary: "Taƙaitawa",
      description: "Bayani",
      clientIndustry: "Masana'antar abokin ciniki",
      projectType: "Nau'in aikin hannu",
      coverImageUrl: "URL na hoton murfin",
      liveUrl: "URL mai rai",
      challengeStatement: "Bayanin ƙalubale",
      solutionOverview: "Taƙaitawar mafita",
      featureHighlights: "Fitattun fasali",
      heroImageUrl: "URL na hoton jarumi",
      clientName: "Sunan abokin ciniki",
      clientRole: "Matsayin abokin ciniki",
      organization: "Ƙungiya",
      quote: "Magana",
      avatarUrl: "URL na avatar",
      imageUrl: "URL na hoto",
      altText: "Madadin rubutu",
      websiteUrl: "URL na gidan yanar gizo",
      sectionKey: "Maɓallin sashe",
      placement: "Matsayi",
      eyebrow: "Gira",
      subtitle: "Ƙaramin take",
      body: "Jiki",
      ctaPrimaryLabel: "Lakabin CTA na farko",
      ctaPrimaryUrl: "URL na CTA na farko",
      ctaSecondaryLabel: "Lakabin CTA na biyu",
      ctaSecondaryUrl: "URL na CTA na biyu",
      fullName: "Cikakken suna",
      email: "Imel",
      phoneNumber: "Lambar waya",
      companyName: "Sunan kamfani",
      subject: "Maudu'i",
      message: "Saƙo",
      preferredLanguage: "Harshen da aka fi so",
      active: "Yana aiki",
      namespace: "Namespace",
      translationKey: "Maɓallin fassara",
      value: "Ƙima",
      groupName: "Sunan ƙungiya",
      key: "Maɓalli",
      label: "Lakabi",
      url: "URL",
      location: "Wuri",
      target: "Manufa",
      visible: "Ana iya gani",
      platform: "Dandamali",
      assetType: "Nau'in kadara",
      mimeType: "Nau'in MIME",
      question: "Tambaya",
      answer: "Amsa",
      pageKey: "Maɓallin shafi",
      type: "Nau'i",
      status: "Matsayi",
    },
    resources: {
      services: {
        title: "Sarrafa ayyuka",
        description: "Ƙirƙiri, gyara, da tsara ayyukan da ake bayarwa.",
        searchPlaceholder: "Nemo ayyuka",
      },
      portfolioProjects: {
        title: "Sarrafa ayyukan hannu na portfolio",
        description: "Kula da nazarin harka da nuna ayyukan.",
        searchPlaceholder: "Nemo ayyukan hannu na portfolio",
      },
      productBlueprints: {
        title: "Sarrafa tsare-tsaren kayayyaki",
        description: "Sarrafa abun cikin saukar tsarin kayayyaki.",
        searchPlaceholder: "Nemo tsare-tsaren kayayyaki",
      },
      testimonials: {
        title: "Sarrafa shaidar",
        description: "Tsara maganganun abokan ciniki da abun cikin amana.",
        searchPlaceholder: "Nemo shaidar",
      },
      clientLogos: {
        title: "Sarrafa tambarin abokan ciniki",
        description: "Kula da tambarin amana da alamun abokan hulɗa.",
        searchPlaceholder: "Nemo tambarin abokan ciniki",
      },
      homepageSections: {
        title: "Sarrafa sassan shafin gida",
        description: "Sarrafa abun cikin jarumi da na talla na shafin gida.",
        searchPlaceholder: "Nemo sassan shafin gida",
      },
      contactSubmissions: {
        title: "Sarrafa abubuwan tuntuɓa da aka gabatar",
        description: "Bita da sabunta abubuwan da aka gabatar na shigowa.",
        searchPlaceholder: "Nemo abubuwan tuntuɓa da aka gabatar",
      },
      newsletterSubscribers: {
        title: "Sarrafa masu biyan kuɗi na wasiƙar labarai",
        description: "Bita da sarrafa bayanan masu biyan kuɗi.",
        searchPlaceholder: "Nemo masu biyan kuɗi na wasiƙar labarai",
      },
      translations: {
        title: "Sarrafa fassarori",
        description: "Kula da kwafin tsarin gida da ƙimar ƙamus.",
        searchPlaceholder: "Nemo fassarori",
      },
      siteSettings: {
        title: "Sarrafa saitunan shafi",
        description: "Sarrafa ƙimar matakin shafi waɗanda za a iya daidaitawa.",
        searchPlaceholder: "Nemo saitunan shafi",
      },
      navigationItems: {
        title: "Sarrafa kewayawa",
        description: "Gyara abubuwan kewayawa na kai da ƙafa.",
        searchPlaceholder: "Nemo abubuwan kewayawa",
      },
      socialLinks: {
        title: "Sarrafa hanyoyin sadarwar zamantakewa",
        description: "Sarrafa hanyoyin sadarwar zamantakewa da ake iya gani.",
        searchPlaceholder: "Nemo hanyoyin sadarwar zamantakewa",
      },
      mediaAssets: {
        title: "Sarrafa kayan watsa labarai",
        description:
          "Kula da hotuna da nassoshi na watsa labarai waɗanda za a iya sake amfani da su.",
        searchPlaceholder: "Nemo kayan watsa labarai",
      },
      faqs: {
        title: "Sarrafa tambayoyin da ake yawan yi",
        description: "Sarrafa tambayoyin da ake yawan yi ta shafi ko sashe.",
        searchPlaceholder: "Nemo tambayoyin da ake yawan yi",
      },
      categories: {
        title: "Sarrafa rukunoni",
        description:
          "Tsara rukunoni waɗanda za a iya sake amfani da su a faɗin sassan abun ciki.",
        searchPlaceholder: "Nemo rukunoni",
      },
    },
  },
  portfolioPage: {
    title: "Ayyukan da aka zaɓa da nazarce-nazarce",
    description:
      "Bincika gidajen yanar gizo, dandamali, dashboards, da tsarin kasuwanci waɗanda aka gina don taimaka wa alamomi su haɓaka akan layi.",
    empty: "Babu ayyukan hannu na portfolio da ake da su tukuna.",
    untitled: "Aikin Hannu marar Suna",
    noSummary: "Babu taƙaitaccen aikin hannu.",
    project: "Aikin Hannu",
    loadError: "An kasa loda fayil ɗin ayyuka.",
  },
  servicesPage: {
    title: "Ayyuka waɗanda ke haɓaka kasuwancinku",
    description:
      "Daga gidajen yanar gizo na musamman zuwa cikakkun tsarin sarrafa kansa, muna gina kayayyakin dijital waɗanda ke taimaka muku yin aiki da wayo, sayarwa da yawa, da haɓakawa da sauri.",
    empty: "Babu ayyuka da ake da su tukuna.",
    untitled: "Sabis marar suna",
    descriptionFallback: "Cikakkun bayanan sabis zasu bayyana anan.",
    viewDetails: "Duba cikakkun bayanan sabis",
    bannerTitle: "Ayyuka masu araha ga kowane kayayyakin da muke ginawa",
    bannerText:
      "Ana iya gina kowane tsarin kayayyaki akan dandamalinmu azaman cikakken sabis. Muna ba da farashi mai sassauƙa - daga ƙaddamar da MVP farawa daga",
    bannerTextEnd:
      "zuwa cikakkun dandamali na masana'antu. Kuna biyan kuɗi kawai don abin da kasuwancinku ke buƙata da gaske.",
    ctaEyebrow: "Shirye don farawa?",
    ctaTitle: "Faɗa mana abin da kuke buƙatar ginawa",
    ctaDescription:
      "Za mu haɗa ku da fakitin sabis ɗin da ya dace kuma mu ba ku jadawalin lokaci da farashi bayyananne - babu ɓoyayyun kuɗaɗe.",
    ctaButton: "Sami shawara kyauta",
  },
  productsPage: {
    title: "Tsarukan kayayyaki don manyan kasuwanci",
    description:
      "Bincika tsarin kayayyakin dijital da aka shirya don ginawa don gidajen yanar gizo, tsarin ajiya, hanyoyin shiga, makarantu, kasuwancin yanar gizo, da dashboards na kasuwanci.",
    empty: "Babu kayayyaki da ake da su tukuna.",
    untitled: "Kayayyaki marar suna",
    noSummary: "Babu taƙaitaccen bayanin kayayyaki.",
    blueprint: "Tsari",
    viewBlueprint: "Duba tsari",
  },
  products: {
    notFound: "Ba a sami samfur ba.",
    untitled: "Samfur marar Suna",
    noSummary: "Babu taƙaitaccen bayanin samfur.",
    eyebrow: "Tsarin Samfur",
    challenge: "Kalubale",
    solution: "Magani",
    features: "Fitattun Siffofi",
    keyBenefits: "Muhimman Fa'idodi",
    useCases: "Yanayin Amfani",
    targetUsers: "Masu Amfani da aka Nufa",
    techStack: "Tarin Fasaha",
    timeline: "Tsarin Lokaci da aka Ƙidaya",
    ctaEyebrow: "Gina da haske",
    ctaTitle: "Shirya don juya wannan tsarin zuwa samfur na gaske?",
    ctaDescription:
      "Bari InkFront ya taimake ku canza ra'ayinku zuwa ingantaccen dandamalin kasuwanci.",
    placeholderIcon: "🧩",
  },
  clientsPage: {
    title: "Labarun abokan ciniki, alamun amana, da shaidar kasuwanci",
    description:
      "Ra'ayoyi daga hulɗar abokan ciniki da ƙungiyoyin da aka tallafa da gidajen yanar gizo, kayayyakin dijital, da tsarin kasuwanci.",
    testimonialsEyebrow: "Shaidar",
    testimonialsTitle:
      "Abin da abokan ciniki ke cewa game da aiki tare da InkFront",
    testimonialsDescription:
      "Ra'ayi na gaske ya kamata ya taimaka wa abokan ciniki na gaba su fahimci inganci, haske, da ƙwarewar aikin ku.",
    noTestimonials: "Babu shaidar da ake da su tukuna.",
    defaultQuote: "Babu rubutun shaida.",
    anonymous: "Abokin Ciniki da Ba a San Sunansa ba",
    client: "Abokin ciniki",
    logosTitle: "Ƙungiyoyi da alamomin abokan ciniki amintattu",
    noLogos: "Babu tambarin abokan ciniki da ake da su tukuna.",
    ctaEyebrow: "Shiga labarin nasara na gaba",
    ctaTitle: "Bari mu gina dandamalin ƙwararru don alamar ku.",
    ctaDescription:
      "Gidan yanar gizonku ya kamata ya sadar da amana, nuna shaida, kuma ya juya baƙi zuwa tattaunawa ta gaske.",
  },
  blog: {
    eyebrow: "InkFront Blog",
    title: "Ra'ayoyi, dabaru, da ci gaban dijital",
    subtitle:
      "Karanta labarai masu amfani akan gidajen yanar gizo, tsarin ajiya, sarrafa kansa, ganin kasuwanci, da ingantattun ayyukan dijital.",
    readArticles: "Karanta labarai",
    featuredLabel: "Fitattun bayanai",
    noFeatured: "Fitattun labarai zasu bayyana anan da zarar an buga su.",
    latestLabel: "Sabbin labarai",
    latestTitle: "Sabbin tunani don alamar ku",
    clearFilters: "Share matattara",
    featured: "Fitaccen",
    readMore: "Karanta labari",
    readTime: "minti 2 karatu",
    minRead: "minti 2 karatu",
    emptyTitle: "Ba'a sami labarai ba",
    emptyText:
      "Labaran da aka buga don wannan harshe ko matattara zasu bayyana anan.",

    watchVideo: "Kalli bidiyo",
    gallery: "Hotuna",
    articleSummary: "Takaitaccen labari",
    summaryFallback:
      "Hankali mai amfani daga InkFront ga masu kasuwanci da ƙungiyoyin dijital.",
    discussProject: "Tattauna aikin ku",
    moreFeatured: "Ƙarin fitattun",
    relatedLabel: "Karatu mai alaƙa",
    relatedTitle: "Ci gaba da bincike",
    viewAll: "Duba duka",
    noRelated: "Ƙarin labarai masu alaƙa za su bayyana nan ba da jimawa ba.",
    notFoundTitle: "Ba a sami labari ba",
    notFoundText: "Wataƙila an cire wannan labarin ko ba a buga shi ba.",
    backToBlog: "Koma zuwa bulogi",
    fromOurBlog: "Daga shafin mu",
    latestInsights: "SABABBU BAYANAI",
  },
  pages: {
    home: {
      eyebrow: "InkFront Digital Systems",
      heroTitle: "Gina ingantaccen kasancewar dijital don kasuwancinku",
      heroSubtitle:
        "InkFront yana tsarawa da gina gidajen yanar gizo, shafukan kayayyaki, hanyoyin shiga, da dandamali na kasuwanci waɗanda ke taimaka wa alamar ku ta zama amintacciya, canza masu nema, da haɓaka akan layi.",
      heroPanelEyebrow: "An gina don haɓaka",
      heroPanelTitle:
        "Gidajen yanar gizo, dashboards, hanyoyin shiga, da tsare-tsaren shirye don ƙaddamarwa.",
      heroPanelItem1: "Gaba",
      heroPanelItem2: "Baya",
      heroPanelItem3: "Abun ciki",
      heroPanelStatus: "Mai kyau",
      statsProjects: "Ayyukan hannu",
      statsServices: "Ayyuka na asali",
      statsBusiness: "Mai da hankali kan kasuwanci",
      statsSystem: "Tsarin kasuwanci mai rai",
      systemsTitle: "Gidajen yanar gizo. Hanyoyin shiga. Kayayyaki.",
      whatWeDo: "Abin da muke yi",
      processEyebrow: "Yadda muke aiki",
      processTitle:
        "Tsari mai haske daga ra'ayi zuwa tsarin shirye don ƙaddamarwa.",
      processDescription:
        "Ba wai kawai muna tsara fuskar allo ba. Muna tsara tsarin kasuwanci, abun ciki, bayanan baya, sarrafa gudanarwa, da ƙwarewar mai amfani tare.",
      productDirection: "Jagorancin kayayyaki",
      productTitle:
        "Juya ra'ayinku zuwa tsarin kasuwanci bayyananne wanda za a iya ginawa.",
      productSubtitle:
        "Yi amfani da tsare-tsaren kayayyaki na InkFront don ayyana shafuka, fasali, abun ciki, tsarin gudanarwa, da jagorancin ƙaddamarwa kafin ci gaba ya fara.",
      selectedWork: "Ayyukan da aka zaɓa",
      clientConfidence: "Amincin abokin ciniki",
      testimonialSubtitle:
        "Sadarwa bayyananna, ƙira ta zamani, da tsare-tsare waɗanda ke sauƙaƙa sarrafa kasuwancinku.",
      trustedBy: "Amintattu daga alamun da ke haɓaka",
      ready: "Shirye lokacin da kuke",
      managed: "An sarrafa",
      service: "Sabis",
      serviceDescription:
        "Sabis na ƙwararru wanda aka ƙera don tallafawa kasuwancinku.",
      productBlueprint: "Tsarin Kayayyaki",
      productBlueprintSummary:
        "Tsara tayin, shafuka, fasali, da tafiyar abokin ciniki kafin ci gaba ya fara.",
      portfolioProject: "Aikin hannu na portfolio",
      portfolioSummary:
        "Aikin dijital mai tsafta wanda aka gina don tasirin kasuwanci.",
      client: "Abokin ciniki",
      happyClient: "Abokin Ciniki Mai Farin Ciki",
      defaultQuote:
        "Dandamali mai walƙiya wanda ke taimaka wa kasuwanci ya zama mafi ƙwararru akan layi.",
      bannerTitle:
        "Kuna buƙatar gidan yanar gizo, hanyar shiga, tsarin ajiya, ko cikakken dandamalin kasuwanci?",
      bannerText:
        "InkFront na iya taimaka muku motsawa daga ra'ayoyin da suka watsu zuwa tsari mai walƙiya mai shafuka, abun ciki, bayanan baya, tantancewa, kayan aikin gudanarwa, da ƙwarewar jama'a mai amsawa.",
      processSteps: {
        discover: {
          title: "Ganowa",
          text: "Muna bayyana burin kasuwancinku, masu sauraro, ayyuka, da nau'in tsarin da kuke buƙata.",
        },
        structure: {
          title: "Tsarawa",
          text: "Muna tsara shafuka, kwararar abun ciki, buƙatun gudanarwa, tafiyar mai amfani, da ƙirar bayanan baya.",
        },
        build: {
          title: "Ginawa",
          text: "Muna haɓaka gaba, baya, sarrafa abun ciki, tantancewa, da haɗin kai.",
        },
        launch: {
          title: "Ƙaddamarwa",
          text: "Muna goge amsawa, aiki, tushen SEO, gwaji, da shirye-shiryen turawa.",
        },
      },
      logoFallback: {
        websites: "Gidajen yanar gizo",
        portals: "Hanyoyin shiga",
        bookings: "Ajiya",
        dashboards: "Dashboards",
        stores: "Shaguna",
      },
      fallbackServices: {
        webDev: "Haɓaka Gidan Yanar Gizo",
        webDevSummary:
          "Gidajen yanar gizo masu sauri, masu amsawa, shirye don SEO waɗanda aka gina don taimaka wa kasuwancinku ya zama amintacce.",
        webDevTimeline: "Kwanaki 7–21",
        webDevHighlights:
          "Shafukan saukowa, gidajen yanar gizo na kamfani, tsarin SEO, kwararar sadarwa",
        software: "Software na Musamman",
        softwareSummary:
          "Dashboards, tsarin ajiya, hanyoyin shiga, da kayan aikin ciki don ayyuka na gaske.",
        softwareTimeline: "Makonni 3–8",
        softwareHighlights:
          "Fasfo na gudanarwa, hanyoyin shiga, kwararar ajiya, dashboards rahoto",
        strategy: "Dabarun Dijital",
        strategySummary:
          "Jagorancin kayayyaki bayyananne, kwararar juyawa, da shirin ƙaddamarwa don alamar ku.",
        strategyTimeline: "Kwanaki 3–10",
        strategyHighlights:
          "Tsarin tayin, tsarin mazurari, taswirar shafi, shirin ƙaddamarwa",
        ecommerce: "Tsarin Kasuwancin E-commerce",
        ecommerceSummary:
          "Shagunan kan layi, kasidar kayayyaki, kwararar oda, da abubuwan siyayya masu shirye don biya.",
        ecommerceTimeline: "Makonni 2–6",
        ecommerceHighlights: "Kayayyaki, darusa, biya, sarrafa oda",
        booking: "Dandamalin Ajiya",
        bookingSummary:
          "Tsarin alƙawari, sabis, sufuri, da ajiya tare da ƙwarewar mai amfani mai santsi.",
        bookingTimeline: "Makonni 2–5",
        bookingHighlights: "Jadawalin, fom, sarrafa gudanarwa, sanarwa",
        training: "Koyarwar Coding & Fasaha",
        trainingSummary:
          "Koyarwa mai amfani ga ƙungiyoyi, ɗalibai, da masu farawa waɗanda ke son fahimtar software na zamani.",
        trainingTimeline: "Mai sassauƙa",
        trainingHighlights:
          "React, Spring Boot, haɓaka gidan yanar gizo, koyo na tushen aiki",
      },
      fallbackProjects: {
        agencySite: "Dandamalin Gidan Yanar Gizo na Hukuma",
        agencySiteSummary:
          "Gidan yanar gizo na kamfani mai walƙiya tare da shafukan ayyuka da kama masu nema.",
        bookingSystem: "Tsarin Sarrafa Ajiya",
        bookingSystemSummary:
          "Kwararar ajiya mai tsafta don kasuwancin tushen sabis.",
        clientPortal: "Hanyar Shiga ta Abokin Ciniki",
        clientPortalSummary:
          "Ƙwarewar dashboard mai aminci ga abokan ciniki da masu gudanarwa.",
      },
      fallbackProducts: {
        blueprint: "Tsarin Kayayyaki",
        blueprintSummary:
          "Tsara ra'ayinku, tsarin samun kuɗi, tafiyar mai amfani, da taswirar gina kafin ci gaba.",
        businessSite: "Kayan Gidan Yanar Gizo na Kasuwanci",
        businessSiteSummary:
          "Tsarin gidan yanar gizo na musamman don alamomin da suke buƙatar aminci, shafuka, da kama masu nema.",
        portalKit: "Kayan Hanyar Shiga ta Abokin Ciniki",
        portalKitSummary:
          "Tushen hanyar shiga abokin ciniki mai aminci don dashboards, bayanan martaba, buƙatun, da ayyukan gudanarwa.",
      },
      fallbackTestimonials: {
        client1: {
          name: "Abokin Ciniki na InkFront",
          role: "Mai Kasuwanci",
          quote:
            "Dandamalin ya ba alamar mu kasancewar ƙwararru akan layi kuma ya sauƙaƙa sarrafa tambayoyi.",
        },
        client2: {
          name: "Abokin Ciniki na Haɓaka",
          role: "Wanda Ya Kafa",
          quote:
            "Ƙira mai tsafta, isarwa da sauri, da tsarin da zai iya haɓaka tare da kasuwancinmu.",
        },
      },
    },
    about: {
      title: "Muna gina tsare-tsaren dijital don kasuwancin zamani",
      subtitle:
        "InkFront na taimaka wa kasuwanci su ƙaddamar da manyan gidajen yanar gizo, shafukan samfur, dashboards, hanyoyin shiga, da dandamalin dijital masu haɗin baya.",
      eyebrow: "Game da InkFront",
      storyEyebrow: "Labarin mu",
      storyTitle: "Dalilin da yasa InkFront yake wanzuwa",
      storyFallback:
        "InkFront ya wanzu don taimaka wa kasuwanci su matsa daga warwatse ra'ayoyi da rashin ƙarfi akan layi zuwa ingantattun dandamali na dijital, amintattu, kuma masu iya haɓakawa.",
      missionTitle: "Manufar mu",
      missionFallback:
        "Don tsarawa da gina tsare-tsaren dijital masu amfani waɗanda ke taimaka wa kasuwanci sadarwa a fili, aiki da kyau, da haɓaka da tabbaci.",
      visionTitle: "Hangen mu",
      visionFallback:
        "Don zama amintaccen abokin haɗin gwiwar fasaha ga kasuwanci waɗanda ke son ƙira mai tsafta, tsayayyen tsarin baya, da haɓakar dijital na dogon lokaci.",
      valuesTitle: "Ƙimomin mu",
      valuesFallback:
        "Muna ɗaukar bayyananniyar fahimta, dogaro, tasirin kasuwanci, aiwatarwa mai tsafta, da sadarwa ta gaskiya da muhimmanci.",
      howWeWorkEyebrow: "Yadda muke aiki",
      howWeWorkTitle:
        "Muna juya burin kasuwanci zuwa tsararrun tsare-tsaren dijital.",
      multilingualReady: "Shirye don harsuna da yawa",
      imagePlaceholder: "Roka",
      workSteps: {
        understand: {
          title: "Fahimci kasuwancin",
          body: "Muna bayyana tayin, masu sauraro, shafuka, ayyuka, da burin kasuwanci kafin ginawa.",
        },
        design: {
          title: "Zana ƙwarewar",
          body: "Muna siffanta shimfiɗa, kwararar abun ciki, da tafiyar mai amfani don dandamali ya ji ingantacce kuma mai sauƙin amfani.",
        },
        build: {
          title: "Gina da tsararrun tsare-tsare",
          body: "Muna haɗa gaba, baya, DTOs, ayyuka, APIs, kayan aikin mai gudanarwa, da abun cikin jama'a da aka buga.",
        },
      },
      fallbackTestimonials: {
        t1: {
          role: "Wanda Ya Kafa",
          quote:
            "InkFront ya gina dandamalin sarrafa makarantarmu daidai yadda muka bayyana shi. Zane mai tsafta, baya mai ƙarfi. Malamanmu da iyayenmu suna son amfani da shi.",
        },
        t2: {
          role: "Shugaba",
          quote:
            "Muna buƙatar tsarin aikawa da sauri. Ƙungiyar ta isar da dashboard ɗin dabaru mai aiki a cikin ƙasa da makonni 5. Yanzu shine jigon ayyukanmu na yau da kullun.",
        },
        t3: {
          role: "Manajan Darakta",
          quote:
            "Kasuwarmu ra'ayi ne kawai. InkFront ya mayar da shi dandamalin masu siyarwa da yawa mai rai tare da escrow, sarrafa kataloji, da kayan aikin mai gudanarwa masu tsafta.",
        },
        t4: {
          role: "CTO",
          quote:
            "Tsarin API na ƙwararru da aikin gaba mai tsafta. Suna fahimtar dabarun kasuwanci, ba lambar kawai ba. Muna aiki tare fiye da shekara guda.",
        },
      },
      fallbackFaqs: {
        f1: {
          question: "Wadanne nau'ikan kasuwanci kuke aiki da su?",
          answer:
            "Muna aiki da masu farawa, SMEs, hukumomi, makarantu, kamfanonin fintech, kamfanonin dabaru, da kasuwancin tushen sabis waɗanda ke buƙatar ingantaccen kasancewar dijital da tsare-tsaren aiki.",
        },
        f2: {
          question: "Yaya tsawon lokacin da ake ɗauka don kammala aikin?",
          answer:
            "Jadawalin ya dogara da girman - gidan yanar gizo na kasuwanci yana ɗaukar makonni 2-4, yayin da cikakken dandamali na musamman zai iya ɗaukar makonni 8-16. Muna ba da ƙididdiga masu haske yayin matakin dabaru.",
        },
        f3: {
          question: "Kuna bayar da tallafi bayan ƙaddamarwa?",
          answer:
            "Ee. Kowane aikin ya haɗa da lokacin garanti kuma muna ba da tsare-tsaren kulawa mai gudana don kiyaye dandamalinku yana sabuntawa, amintacce, kuma yana aiki da kyau.",
        },
        f4: {
          question: "Za ku iya aiki da ƙungiyar data kasance ko kayan aikina?",
          answer:
            "Tabbas. Muna haɗawa da tafiyar ayyukanku na yanzu, APIs, ma'ajin bayanai, da kayan aikin ɓangare na uku. Muna gina bisa abin da kuke da shi, ba maye gurbin komai ba.",
        },
        f5: {
          question: "Nawa ne aikin da aka saba kashewa?",
          answer:
            "Fakitin MVP ɗinmu suna farawa daga $2,500 don cikakken gidan yanar gizo na kasuwanci. Dandamali na musamman suna daga $5,000–$25,000+ dangane da sarƙaƙiya. Kowane aikin yana samun shawarar farashi da aka ƙayyade a gaba.",
        },
        f6: {
          question: "Waɗanne fasahohi kuke amfani da su?",
          answer:
            "Muna ginawa da React, Spring Boot, PostgreSQL, Docker, da kayan aikin girgije na zamani. Don ƙa'idodin wayar hannu, muna amfani da React Native ko Flutter. Muna zaɓar tarin da ya dace don buƙatunku.",
        },
      },
      stats: {
        engineers: "Injiniyoyin software",
        experience: "Shekarun gwaninta",
        focus: "Mai da hankali ga Afirka",
      },

      bullets: {
        item1:
          "Goyan bayan ƙungiyar injiniyoyin software masu sassauƙa na 5 zuwa 15.",
        item2: "Kyakkyawar fahimtar dabarun kasuwanci da ƙwarewar mai amfani.",
        item3:
          "Fiye da shekaru 10 na gwaninta a fadin tsarin yanar gizo da software.",
        item4:
          "Mai da hankali ga Najeriya da farko, tare da tunanin ci gaban Afirka.",
      },

      engineeringDepth: {
        eyebrow: "Zurfin injiniyanci",
        title: "Muna haɗa ƙwarewar software tare da fahimtar kasuwanci.",
        description:
          "Aikinmu ba game da lambar kawai ba ne. Muna tunanin yadda kasuwanci ke karɓar jagorori, sarrafa abokan ciniki, gabatar da ayyuka, sarrafa abun ciki, sarrafa bayanai, da ƙirƙirar ƙwarewa mai sauƙi ga masu amfani.",
        item1: {
          title: "Dabarun kasuwanci na farko",
          body: "Kafin ginawa, muna fahimtar ayyukanku, masu amfani, ayyuka, ayyukan aiki, da tsarin samun kuɗi.",
        },
        item2: {
          title: "Ƙwarewar mai amfani mai tsabta",
          body: "Muna tsara shafuka, dashboards, fom, da kwararar ruwa waɗanda suke da sauƙi, na musamman, da sauƙin amfani.",
        },
        item3: {
          title: "Injiniyanci abin dogaro",
          body: "Muna ginawa tare da tsarin gaba mai tsabta, ayyukan baya, DTOs, APIs, tantancewa, da dabarun bayanai.",
        },
        item4: {
          title: "Fahimtar kasuwar Afirka",
          body: "Muna ginawa tare da fahimtar gaskiyar kasuwancin Najeriya da Afirka: amana, sauri, amfani da wayar hannu, bayyananne, da araha.",
        },
      },

      howWeWork: {
        eyebrow: "Yadda muke aiki",
        title: "Daga ra'ayi zuwa tsarin dijital mai kyau.",
        description:
          "Kowane aiki yana bin tsari bayyananne don dandamali na ƙarshe ya kasance ba kawai kyakkyawa ba, amma mai amfani, mai kulawa, kuma mai daidaita da yadda kasuwancinku yake aiki.",
      },
    },
    services: {
      notFound: "Ba a sami sabis ba",
      notFoundDesc:
        "Wannan sabis ɗin yana iya zama babu shi ko kuma ba a buga shi ba tukuna.",
      untitled: "Sabis marar suna",
      summaryFallback: "Cikakkun bayanan sabis zasu bayyana anan.",
      requestService: "Nemi wannan sabis ɗin",
      overview: "Ƙididdiga",
      timeline: "Jadawalin lokaci",
      timelineFallback: "Jadawalin lokaci ya dogara da girman aikin.",
      deliverables: "Abin da kuke samu",
      deliverablesFallback:
        "Dabarar da ta dace, jagorar ƙira mai inganci, abun ciki mai haɗin baya, UI mai amsawa, da tsarin isarwa mai da hankali kan kasuwanci.",
      moreServices: "Ƙarin ayyuka",
      otherServices: "Sauran hanyoyin da InkFront zai iya taimakawa",
      service: "Sabis",
      viewService: "Duba sabis",
      ctaEyebrow: "Shirye don ginawa?",
      ctaTitle: "Bari mu yi magana game da aikin ku.",
      ctaDescription:
        "Faɗa mana abin da kuke buƙata. Za mu ba da shawarar fakitin sabis ɗin da ya dace, jadawalin lokaci, da mataki na gaba.",
      ctaButton: "Sami shawara kyauta",
    },

    portfolio: {
      notFound: "Ba a sami aikin hannu ba",
      untitled: "Aikin hannu marar suna",
      noDescription: "Babu bayanin aikin hannu.",
      caseStudy: "Nazarin Harka",
      overviewTitle: "Ƙididdigar Aikin Hannu",
      clientIndustry: "Masana'antu",
      projectType: "Nau'in Aikin Hannu",
      ctaEyebrow: "Aikin ku na gaba",
      ctaTitle: "Kuna son dandamali da aka gina da wannan matakin walƙiya?",
      ctaDescription:
        "Bari mu tsara mu gina gidan yanar gizo na kasuwanci ko tsari wanda ke taimaka wa alamar ku ta fice.",
    },

    blueprint: {
      challenge: "Ƙalubale",
      solution: "Mafita",
      features: "Fasali",
    },

    contact: {
      title: "Bari mu gina tsarin dijital ɗinku na gaba",
      subtitle:
        "Aika cikakkun bayanan aikin ku kai tsaye zuwa ga ƙungiyar mai gudanarwa ta InkFront. Za a adana tambayarku, a bi diddigi, kuma a bi ta daga CRM na mai gudanarwa.",
      inquiry: "Tambayar aiki",
      sendMessage: "Aiko mana da saƙo",
      description:
        "Faɗa mana abin da kuke son ginawa. Haɗa burin aikin ku, jadawalin lokaci da aka fi so, fasalin da ake buƙata, da kewayon kasafin kuɗi idan kuna da ɗaya.",
      nextSteps: {
        title: "Me zai faru a gaba?",
        description:
          "Ana adana saƙonku a cikin CRM Tuntuɓar mai gudanarwa, inda ƙungiyar za ta iya bitar sa, sanya shi, sabunta matsayi, da bi diddigi.",
      },
      detailsToInclude: {
        title: "Mafi kyawun cikakkun bayanai da za a haɗa",
        description:
          "Haɗa nau'in kasuwancinku, burin aiki, fasalin da ake buƙata, jadawalin lokaci, kewayon kasafin kuɗi, da duk wasu gidajen yanar gizo na misali da kuke so.",
      },
      howWeRespond: {
        title: "Yadda muke amsawa",
        description:
          "Muna bitar buƙatar kuma mu ba da amsa tare da shawarwari masu amfani, iyawar da za ta iya yiwuwa, jadawalin lokaci, da mataki na gaba.",
      },
      storedSafely: {
        title: "An adana cikin aminci",
        description:
          "Ana adana kowace tambaya azaman saƙon CRM don kada wani mai nema ya ɓace a cikin imel, taɗi, ko bayanan hannu.",
      },
    },
    admin: {
      title: "Dashboard na Mai Gudanarwa",
      subtitle: "Shell ɗin mai gudanarwa da aka tabbatar yana aiki.",
      statusLoading: "Ana duba damar mai gudanarwa...",
    },
    notFound: {
      title: "Ba a sami shafi ba",
      message:
        "Shafin da kuke nema babu shi ko kuma yana iya zama an motsa shi.",
    },
  },

  sections: {
    hero: { eyebrow: "Isar da dijital mai inganci" },
    services: {
      title: "Ayyuka",
      description: "Ƙarfin sabis na asali don isar da software na zamani.",
    },
    portfolio: {
      title: "Fayil ɗin Ayyuka",
      description:
        "Ayyukan da aka zaɓa a faɗin kayayyaki, dandamali, da canjin dijital.",
    },
    products: {
      title: "Kayayyaki",
      description:
        "Tsare-tsaren mafita ga ƙungiyoyi waɗanda ke son haske kafin cikakken aiwatarwa.",
    },
    testimonials: {
      eyebrow: "Shaidar",
      title: "Abin da abokan ciniki ke cewa",
      description:
        "Shaida daga mutane da kasuwanci waɗanda suka amince da InkFront.",
      emptyQuote: "Babu rubutun shaida.",
      anonymous: "Abokin Ciniki da Ba a San Sunansa ba",
      client: "Abokin ciniki",
      quoteMark: "Magana",
    },
    trust: {
      title: "Abokan ciniki & amana",
      description: "Hoton gaggawa na ƙungiyoyi da alamomi a bayan aikin.",
    },
    newsletter: {
      title: "Ku kasance kusa da abin da muke ginawa",
      description:
        "Samu zurfin tunani game da kayayyaki da software a cikin akwatin saƙonku.",
    },
    faq: {
      eyebrow: "FAQ",
      title: "Tambayoyin da ake yawan yi",
      description:
        "Amsoshi masu taimako don tambayoyin gama gari game da yadda muke aiki.",
      defaultQuestion: "Tambaya",
      defaultAnswer: "Amsa babu.",
    },
  },

  cta: {
    home: {
      title:
        "Shirye don gina wani abu mai haske, mai amfani, kuma mai iya haɓakawa?",
      description:
        "Bari mu siffata sigar ta gaba ta kayayyakinku, dandamalinku, ko ƙwarewar dijital ɗinku.",
    },
    about: {
      title: "Shirye don gina dandamalin dijital ɗinku?",
      description:
        "Bari mu ƙirƙiri gidan yanar gizo na ƙwararru, shafin kayayyaki, hanyar shiga, ko tsarin kasuwanci na musamman don alamar ku.",
    },
    portfolio: {
      title: "Kuna son sakamako kamar wannan don kasuwancinku?",
      description:
        "Bari mu tsara kayayyakin, dandamali, ko tsarin haɓaka wanda ya dace da matakinku na gaba.",
    },
    products: {
      title: "Kuna buƙatar tsari da aka kera don yanayin amfaninku?",
      description:
        "Zamu iya daidaita dabarun mafita zuwa wani abu mai amfani ga mahallinku.",
    },
    clients: {
      title: "Kuna neman abokin tarayya da ƙungiyarku za ta iya amincewa?",
      description:
        "Muna haɗa haske, haɗin kai, da aiwatarwa ba tare da hayaniya mara amfani ba.",
    },
    notFound: {
      title: "Bari mu dawo da ku kan hanya",
      description:
        "Fara daga shafin gida ko bincika ayyukanmu da ayyukan kwanan nan.",
    },
  },

  states: {
    loadingPortfolio: "Ana loda fayil ɗin ayyuka...",
    error: "Wani abu ya faru ba daidai ba",
    loadingPage: "Ana loda shafi...",
    emptyGeneric: "Babu wani abun ciki da ake samu tukuna.",
    errorTitle: "Wani abu ya faru ba daidai ba",
    errorGeneric: "Ba za mu iya loda wannan shafin ba a yanzu.",
    notAvailable: "Babu samuwa",
  },

  forms: {
    newsletter: {
      namePlaceholder: "Sunanka",
      emailPlaceholder: "Shigar da imel ɗinku",
      subscribe: "Biyan kuɗi",
      success: "Biyan kuɗi ya yi nasara.",
      error: "Biyan kuɗi ya kasa. Da fatan za a sake gwadawa.",
    },
    contact: {
      fullName: "Cikakken suna",
      fullNamePlaceholder: "Cikakken sunanku",
      email: "Imel",
      emailPlaceholder: "ku@example.com",
      phoneNumber: "Wayar hannu",
      phonePlaceholder: "+234...",
      companyName: "Kamfani / Alama",
      companyPlaceholder: "Sunan kamfani ko alama",
      serviceInterest: "Sha'awar sabis",
      selectService: "Zaɓi sabis",
      preferredLanguage: "Harshen da aka fi so",
      subject: "Maudu'i",
      subjectPlaceholder: "Me kuke buƙata?",
      message: "Saƙo",
      messagePlaceholder:
        "Bayyana aikin ku, jadawalin lokaci, kewayon kasafin kuɗi, shafuka/fasali da ake buƙata, da wace matsala kuke son warwarewa...",
      submit: "Aika saƙo →",
      sending: "Ana aikawa...",
      success:
        "An aika saƙo cikin nasara. Ƙungiyar mai gudanarwa ta InkFront za ta bita kuma ta dawo gare ku nan ba da jimawa ba.",
      error: "An kasa aika saƙo. Da fatan za a sake gwadawa.",
      validationError: "Da fatan za a cika sunanku, imel, maudu'i, da saƙo.",
    },
  },

  cards: {
    blueprint: { viewBlueprint: "Duba tsari" },
  },
};
